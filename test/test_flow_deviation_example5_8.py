...
costs = {
    (0,1): lambda x: 0,
    (1,4): lambda x: x[0]**2/2,
    (0,2): lambda x: 0,
    (2,4): lambda x: x[1]**2/2,
    (0,3): lambda x: 0,
    (3,4): lambda x: x[2]**2/2 + .55*x[2],
}
...
alpha_fn = lambda xp,x_: -1\
    * (xp[0]*(x_[0]-xp[0])+xp[1]*(x_[1]-xp[1])+(.1*x[2]+.55)*(x_[2]-xp[2]))\
    / ((x_[0]-xp[0])**2+(x_[1]-xp[1])**2+.1*(x_[2]-xp[2])**2)

flow_deviation(G, od_paths, od_req, costs, costs_, x0, tol=.1,
